...
def main():
    ...
    gen = Generator(x_dim=args.x_dim, s_dim=args.s_dim, z_dim=args.z_dim, layers=args.dec)
    # gen.train()
    ...
    for epoch in range(args.epoch):
        ...
        for batch in tqdm(range(steps), leave=False, ncols=70, unit='b'):
            for i in range(args.d_iter):
                ...
                # calculate loss
                d_loss = mse_loss(true_scores, ones) + mse_loss(fake_scores2, zeros) # + args.theta3 * mse_loss(reg_scores, zeros) \
        ...
        loss_history.append(f'{g_avg_loss:.4}\t{d_avg_loss:.4}\t{r_avg_loss:.4}\t'
                            f'{g_avg_score:.4}\n')
